...
def load_inference_bundle():
    ...
    try:
        # 보정된 최종 모델 로드
        with open("final_best_model_calibrated.pkl", "rb") as f:
            model = pickle.load(f)
        # 추론에 필요한 모든 정보(임계값, 변수 목록 등) 로드
        with open("inference_bundle.pkl", "rb") as f:
            bundle = pickle.load(f)
        return model, bundle
    except FileNotFoundError:
        st.error("오류: 'final_best_model_calibrated.pkl' 또는 'inference_bundle.pkl' 파일을 찾을 수 없습니다.")
        return None, None
...
with st.sidebar:
    ...
    with st.expander("기본 정보", expanded=True):
        ...
        sm_presnt = st.selectbox("현재 흡연 여부", options=[1, 2, 3], index=2, format_func=lambda x: {1:'매일', 2:'가끔', 3:'비흡연'}[x])
    ...
    with st.expander("병력 및 가족력"):
        ...
        HE_HPfh1 = st.selectbox("고혈압 가족력(부)", options=[1, 0], index=1, format_func=lambda x: "예" if x==1 else "아니오", key='hp_father')
        HE_DMfh1 = st.selectbox("당뇨병 가족력(부)", options=[1, 0], index=1, format_func=lambda x: "예" if x==1 else "아니오", key='dm_father')
...
if model and bundle:
    ...
    input_data = {
        ...
        'DI2_dg': DI2_dg, 'sm_presnt': sm_presnt, 'HE_HPfh1': HE_HPfh1, 'HE_DMfh1': HE_DMfh1,
    ...
    # 최적 임계값 적용
    threshold = bundle.threshold_global
    ...
    try:
        # --- ★★★ 오류 수정: .estimator 제거 ★★★ ---
        # 로드된 모델이 이미 Stacking 모델이므로 바로 named_estimators_에 접근합니다.
